...
async def async_setup_entry(hass, config_entry, async_add_entities):
    ...
    entry: StateManager = hass.data[DOMAIN][config_entry.entry_id]
    manager = entry.manager
    exclude_devices: list[str] = entry.entry_options[CONF_EXCLUDE]
    new_entities = []
        ...
        if did not in exclude_devices:
            ...
            if isinstance(device, HomePilotCover):
                ...
                if cover.has_ventilation_position_config:
                    ...
                    new_entities.append(HomePilotVentilationPositionEntity(entry, device))
...
class HomePilotTemperatureThresholdEntity(HomePilotEntity, NumberEntity):
    ...
    def __init__(
        self, state_manager: StateManager, device: HomePilotThermostat, thresh_number: int
    ) -> None:
        super().__init__(
            state_manager,
            device,
